...
-- EMPLOYEE 테이블에서 부서코드, 부서멸 급여 합 조회
-- 1)
...
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여' ) 성별,
	...
	SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여' ); -- 별칭 사용X(별칭 해석 X)
ORDER BY "인원 수 ";
...
-- EMPLOYEE 테이블에서 직급별 인원수가 5명 이하인 직급코드, 인원수를 조회(직급코드 오름차순)
...
-- ROLLUP : GROUP BY 절에서 가장 먼저 작성되는 컬럼의 중간 집계 처리하는 함수
...
-- CUBE : GROUP BY 절에 작성된 모든 컬럼의 중간 집계 처리하는 함수
...
MINUS
-- 급여가 300만 초과인 사원의 사번, 이름, 부서코드, 급여
SELECT EMP_ID, EMP_NAME, DEPT_CODE, '1' -- 이건 안돼, SALARY랑 타입이 다름 '1'은 문자
FROM EMPLOYEE
WHERE SALARY > 3000000;
