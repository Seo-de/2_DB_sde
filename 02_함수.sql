...
-- 문자열을 앞에서부터 검색하여 첫 번째 B가 나오는 위치
...
SELECT TRIM('#' FROM '#####안녕#####')
FROM DUAL; -- #은 공백이 아니라 생략이 안되던걸 FROM을 붙여서 없애줌
...
SELECT ROUND(123.456, -1) FROM DUAL; -- 소수점 0 번째 자리에서 반올림(0 기본값)
									  -- 소수점 -1자리 표현
									  -- == 1의 자리에서 반올림에서 10의 자리부터 표현

SELECT ROUND(123.456, -2) FROM DUAL; -- 10의 자리에서 반올림에서 100의 자리부터 표현
...
SELECT LAST_DAY('2022-08-01') FROM DUAL; -- 윤년인지 아닌지 볼 때 
...
-- L : 그 지역의 화폐를 써준데(여기서 원화 표시는 백슬래쉬라서 l을 사용)
...
SELECT SYSDATE FROM DUAL; -- 2022-09-01 11:20:41.000

-- 2022/09/01 11:20:41 목요일 -> 이런식으로 만들고 싶으면
...
-- CASE WHEN 조건식 THEN 결과값 // 자바의 IF문
--      WHEN 조건식 THEN 결과값
--      ELSE 결과값
-- END
...
SELECT EMP_NAME, SALARY,
	CASE
		WHEN SALARY >= 5000000 THEN '대' -- if문
		WHEN SALARY >= 3000000 THEN '중' -- else if문
		ELSE '소'
	END "급여 받는 정도"
FROM EMPLOYEE;
...
SELECT COUNT(*) FROM EMPLOYEE; -- EMPLOYEE 테이블의 행의 개수 : 23
