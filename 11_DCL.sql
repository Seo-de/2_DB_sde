...
* 사용자 계정 : 데이터베이스에 대하여 질의, 갱신, 보고서 작성 등의
                ...
                ex) kh_sde계정(각자 이니셜 계정), workbook 등
*/    
...
CREATE USER sde_sample IDENTIFIED BY sample1234;
...
GRANT CREATE SESSION TO sde_sample;
...
GRANT CREATE TABLE TO sde_sample;

ALTER USER sde_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO sde_sample;
...
-- kh_sde   /   sde_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- (sample) kh_sde 계정의 EMPLOYEE 테이블 조회
SELECT * FROM sde.EMPLOYEE;
--> 접근 권한이 없어서 조회 불가

-2. (kh) sde_sample 계정에 EMPLOYEE 테이블 조회 권한 부여
