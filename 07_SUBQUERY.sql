...
-- 사번, 이름, 직급, 급여를 조회하세요
-- 단, > ALL 혹은 < ALL 연산자를 사용하세요 (* ANY와 반대)
...
IN (SELECT JOB_CODE, TRUNC(AVG(SALARY))
	FROM EMPLOYEE
	GROUP BY JOB_CODE);
...
-- 2) 메쿼리 조회 시 ROWNUM을 5이하 까지만 조회
SELETE ROWNUM, EPT_NAME, SALARY
...
FROM EMPLOYEE 
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
GROUP BY DEPT_CODE, DEPT DEPT_TITLE DEPT_ID
ORDER BY 평균급여 DESC ) 
WHERE ROWNUM <= 3;
------------------------------------------------------------------------
