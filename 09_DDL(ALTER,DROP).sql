...
-- DEPARTMENT TABLE 복사(컬럼명, 데이터타입, NOT NULL만 복사)
...
-- LNAME 컬럼 추가(기본값 '한국')
ALTER TABLE DEPT_COPY ADD(LNAME VARCHAR2(30) DEFUALT '한국');
SELECT * FROM DEPT_COPY;
...
-- LANME '한국' -> 'KOREA'로 변경
UPDATE DEPT_COPY SET LANME = DEFAULT
WHERE LNAME = '한국';
...
--> DEPT_COPY 테이블에 PK 추가(컬럼 : DEPT_ID 제약 조건명 :D_COPY_PK)
ALTER TABLE DEPT_COPY ADD CONSTRAINT D_COPY_PK PRIMARY KEY(DEPT_ID);

--> 3. 이름 변경(컬럼, 제약조건, 테이블)

-- 1) 컬럼명 변경 (DEPT_TITLE -> DEPT_NAME)
ALTER TABLE DEPT_COPY RENAME COLUMN DEPT_TITLE TO DEPT_NAMEL;
SELECT * FROM DEPT_COPY;
...
INSERT INTO TB2 VALUES(14, 4); -- 부모한테 4가 없어서 안되

-- TB1 삭제
DROP TABLE TB1;
-- ORA-02449 : 외래 키에 의해 참조되는 고유/ 기본 키가 테이블에 있습니다
--> 해결방법
-- 1) 자식, 부모 테이블 순서로 삭제 -> 자식도 사라짐
...
--> DDL은 수행 시 존재하고 있는 트랜잭션을 보두 DB에 강제 COMMIT 시킴
