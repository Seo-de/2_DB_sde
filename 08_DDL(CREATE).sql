...
-- DML (Data Manifulation Language) : 데이터 조작 언어, 테이블에 데이터를 삽입, 수정, 삭제하는 언어
...
/* 자료형
    ...
    CHAR(크기) : 고정길이 문자형 (2000BYTE) - 학번, 전화번호 / 속도 빠름
        -> ex) CHAR(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공간을 모두 사용. 
        
    VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE) - 이름, 주소, 자기소개 등 / 속도 느림
...
-- 1. NOT NULL 
-- 해당 컬럼에 반드시 값이 기록되어야 하는 경우 사용 (아이디, 비밀번호, 이런거)
...
-- * 제약조건 지정 방법
--  1) 컬럼 레벨 : [CONSRTAINT 제약조건명] 제약조건
--  2) 테이블 레벨 : [CONSRTAINT 제약조건명] 제약조건(컬럼명)
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_SDE.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
--  ORA-01400: NULL을 ("KH_SDE"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- 복합키 설정 되어있어도 하나라도 NULL이 있으면 안됨
----------------------------------------------------------------------------------------------------------------
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE/*(GRADE_CODE)*/ --참조할 컬럼명(PK) / 컬럼레벨
...
-- ORA-02291: 무결성 제약조건(KH_SDE.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
WHERE GRADE_CODE = 30;
-- ORA-02292: 무결성 제약조건(KH_SDE.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- ON DELETE CASCADE(종속) 삭제 옵션이 적용된 테이블 생성
...
ALTER TABLE DEPARTMENT ADD CONSTRAINT LOCATION_ID
